...
class Codegen(config: CodegenConfig) {
  ...
  def apiToMap(path: String, operation: Operation): Map[String, AnyRef] = {
    ...
    if (operation.parameters != null) {
      operation.parameters.foreach(param => {
        ...
        if("json".equals(param.dataType.toLowerCase())){
            param.dataType = "Map"
        }
  ...
  def modelToMap(className: String, model: Model): Map[String, AnyRef] = {
    ...
    model.properties.map(prop => {
      ...
      if (baseType.toLowerCase().equals("json")){
         baseType = "Map"
         propertyDocSchema.`type` = "Map"
      }
      ...
      val properties =
        HashMap(
          ...
          "nameJava" -> {
            config.toVarName(prop._1).replaceAll("^_|[@*#]", "").substring(0,1) + config.camelify(config.toVarName(prop._1).replaceAll("^_|[@*#]", "")).substring(1)
          },
          "nameSingular" -> {
          ...
          "javagetter" -> config.toJavaGetter(prop._1, config.toDeclaration(propertyDocSchema)._1),
          "javasetter" -> config.toJavaSetter(prop._1, config.toDeclaration(propertyDocSchema)._1),
          "isList" -> isList,
          "isMap" -> isMap,
